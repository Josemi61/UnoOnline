...
export default function UserSearchModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        data = data.map(user => ({
          ...user,
          hasSentRequest: sentRequests.includes(user.id)
        }))
  ...
  const handleSendFriendRequest = (targetUserId: string) => {
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      const sentRequests = JSON.parse(localStorage.getItem("sentRequests") || "[]")
      localStorage.setItem("sentRequests", JSON.stringify([...sentRequests, targetUserId]))
    } else {
            ...
            {users.map((user) => (
              <li key={user.id} className="flex items-center justify-between">
                ...
                <div>
                  ...
                  {user.hasSentRequest && (
                    <span className="bg-gray-300 text-black font-bold py-1 px-2 rounded text-sm">
                      Solicitud Enviada
                    </span>
                  )}
