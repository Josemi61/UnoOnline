...
namespace UnoOnline.Repositories
{
    public class FriendshipRepository : IFriendshipRepository
    {
        ...
        public async Task AddRequest(FriendRequest request)
        {
            ...
            {
                Console.WriteLine($"Friend Request already exists: Sender {request.SenderId}, Receiver {request.ReceiverId}");
                return;
            }
        ...
        public async Task<FriendRequest> GetRequestById(int requestId)
        {
            ...
            if (request == null)
            {
                Console.WriteLine($"No Friend Request found with ID {requestId}");
            }
            return request;
        }
        ...
        public async Task<List<User>> GetFriends(int userId)
        {
            return await _context.FriendRequests
                .Where(r => r.ReceiverId == userId && r.Status == RequestStatus.Pending) // Solo solicitudes recibidas y pendientes
                .Select(r => r.Sender) // Obtener los usuarios que enviaron la solicitud
                .ToListAsync();
        }
    }
}
